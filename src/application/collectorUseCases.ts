...
export function buildDiscoveryAudit(options: Pick<CollectorOptions, 'startDate' | 'endDate' | 'marketDuration'>, rawMarkets: Record<string, unknown>[], discoveryResult: { acceptedMarkets: NormalizedMarket[]; rejectedMarkets: RejectedMarket[] }, debug?: { candidateMarketsFetched: number; deduplicatedCandidateMarkets: number; rejectedByReason: Record<string, number>; acceptedByDuration: Record<string, number>; stopReason?: string; searchedExactTitleTermsCount?: number; searchedGenericTermsCount?: number }): Record<string, unknown> {
  ...
  const rejectedBySlug = new Map(discoveryResult.rejectedMarkets.filter((market) => market.marketSlug !== null).map((market) => [market.marketSlug as string, market]));
