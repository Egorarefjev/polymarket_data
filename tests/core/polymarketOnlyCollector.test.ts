...
function market(): NormalizedMarket {
  return {
    marketSlug: "bitcoin-up-or-down-april-30-2026-7pm-et",
    conditionId: "condition",
    question: "Bitcoin Up or Down - April 30, 7PM ET",
    marketDuration: "1h",
    marketStartTimestampMilliseconds: Date.parse("2026-04-30T23:00:00.000Z"),
    marketEndTimestampMilliseconds: Date.parse("2026-05-01T00:00:00.000Z"),
    upTokenId: "up",
    downTokenId: "down",
    targetPrice: 76237.07,
    winner: "up",
    isResolved: true,
    isClosed: true,
    rawOutcomes: '["Up","Down"]',
...
function history(count: number, base: number): PriceHistoryPoint[] {
  return Array.from({ length: count }, (_, index) => ({
    timestampMilliseconds:
      Date.parse("2026-04-30T23:00:00.000Z") + index * 60_000,
...
describe("Polymarket-only collector regression", () => {
  it("builds all Polymarket price points without external primary prices", () => {
    const result = buildNormalizedPricePointsForMarketWithSkipCount({
      market: market(),
      upPriceHistory: history(103, 0.4),
      downPriceHistory: history(103, 0.6),
      requestedFidelityMinutes: 1,
    });
    expect(result.pricePoints).toHaveLength(103);
    expect(
      buildStrategyTrainingRows(result.pricePoints).length,
    ).toBeGreaterThan(0);
    expect(result).not.toHaveProperty(
      "skippedRowsMissingPrimaryPriceBeforeTimestamp",
    );
  ...
  it("range date loop includes start date and excludes end date", () => {
  ...
  it("package.json contains collect:pm:range", () => {
    expect(packageJson.scripts).toHaveProperty(
      "collect:pm:range",
      "npm run collector -- range --market-duration all --write-debug-json false",
    );
  });
});
